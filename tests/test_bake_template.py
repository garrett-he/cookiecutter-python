...
def test_bake_pyinstaller(cookies: Cookies):
    ...
    assert f'poetry run pyinstaller {result.context["project_slug"]}.spec' in result.project_path.joinpath('Makefile').read_text()
    ...
    assert f'poetry run pyinstaller {result.context["project_slug"]}.spec' not in result.project_path.joinpath('Makefile').read_text()
